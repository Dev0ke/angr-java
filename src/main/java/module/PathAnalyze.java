...
public class PathAnalyze {
    ...
    public OnTheFlyJimpleBasedICFG icfg;
    public boolean enableSolve;

    public PathAnalyze(SootMethod entryMethod){
        this.startTime = System.currentTimeMillis();
        this.entryMethod = entryMethod;
        this.icfg = new OnTheFlyJimpleBasedICFG(entryMethod);
//        this.graph = new ExceptionalUnitGraph(this.entryMethod.retrieveActiveBody());
    ...
    public void startAnalyze(){
        ...
        Log.info("[+] PathAnalyze finished in " + (endTime - this.startTime)/1000 + "s");
        ...
        while(curUnit != null) {
            ...
            if (curUnit instanceof JIfStmt ifStmt) {
                ...
                if (result != null) {
                    ...
                    if (enableLazySolve || solveConstraintsSingle(branchState.constraints))  // pre solve to avoid path explosion
                    ...
                    if (enableLazySolve || solveConstraintsSingle(state.constraints)) // pre solve to avoid path explosion
                ...
                FlowState branchState = state.copy();
                if(exp != null){
                    branchState.addConstraint(z3Ctx.mkNot(z3Ctx.mkOr(caseValues.stream().map(ii -> z3Ctx.mkEq(exp, z3Ctx.mkInt(ii.value))).toArray(Expr[]::new))));
                    if(enableLazySolve || solveConstraintsSingle(state.constraints)) {
...
//                    }
    ...
    public Expr handleAppOpAPI(InvokeExpr expr, FlowState state){
        List<Value> args = expr.getArgs();
        String methodName = expr.getMethod().getName();
        if(CheckAppOpAPI.getAllMethodNameByClassName("android.app.AppOpsManager").contains(methodName) || methodName.equals("noteOp")){
            // get APPOP STR
            String appOPSTR;
    ...
    //============================================================================================
    public boolean printSimplifyConstaints(List<Expr> constraints) {
        Log.info("================== [Solve] ========================");
        Solver s = this.z3Ctx.mkSolver();
        boolean sat = false;

        // 合并约束
        Expr combinedConstraints = this.z3Ctx.mkAnd(constraints.toArray(new BoolExpr[0]));
        Log.info( combinedConstraints.toString());

        // 使用 ctx-solver-simplify
//        Tactic ctxSimplify = this.z3Ctx.mkTactic("ctx-solver-simplify");
//        Goal goal = this.z3Ctx.mkGoal(true, false, false);
//        goal.add((BoolExpr) combinedConstraints);
//        ApplyResult simplifiedResult = ctxSimplify.apply(goal);
//
//        BoolExpr[] finalConstraints = simplifiedResult.getSubgoals()[0].getFormulas();
//
//        // 添加约简后的约束到求解器
//        for (BoolExpr c : finalConstraints) {
//            s.add(c);
//            Log.info("Final constraint: " + c.toString());
//        }
//
//        // 检查是否可满足
//        sat = s.check() == Status.SATISFIABLE;
//        Log.info("Final SAT status: " + sat);
        Log.info("===================================================");
        return sat;
    }
...
//                blockingClause.add(this.z3Ctx.mkNot(this.z3Ctx.mkEq(entry.getKey(), entry.getValue())));
//            }
...
//        }
    ...
    class FlowState {
        ...
        Stack<Unit> callStack;
        Stack<DirectedGraph<Unit>> cfgStack;
        List<List<Expr>> paramList;
        Map<SootClass,Map<Value,Expr>> staticMaps;
